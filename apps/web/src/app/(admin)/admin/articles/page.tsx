...
export default function AdminArticlesPage() {
  ...
  const hardDeleteMutation = useHardDeleteArticle();
  ...
  const handleHardDelete = async () => {
    ...
    try {
      ...
      toast.success('Article définitivement supprimé');
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between rounded-xl bg-gradient-to-r from-primary/5 via-primary/10 to-transparent p-6">
        <div>
          <h1 className="text-2xl font-bold">{showTrash ? 'Corbeille - Articles' : 'Articles'}</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {showTrash
              ? 'Articles supprimés. Restaurez ou supprimez définitivement.'
              : 'Rédigez et publiez des articles pour le blog.'}
          </p>
        </div>
        {!showTrash && (
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="mt-3 text-sm font-medium">
                {showTrash ? 'La corbeille est vide' : 'Aucun article trouvé'}
              </p>
      ...
      <DeleteConfirmDialog
        ...
        description="L'article sera déplacé dans la corbeille. Vous pourrez le restaurer ultérieurement."
      />
      ...
      <DeleteConfirmDialog
        ...
        description="Cette action est irréversible. L'article et ses images seront définitivement supprimés."
      />
    </div>
  );
}
